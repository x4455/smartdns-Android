...
- 如果在 Magisk Manager 中刷入后续更新，会尝试继承脚本配置。但有可能因为参数变化而导致意料之外的错误，届时请自行重置 lib.sh 。
- 建议安装 Busybox for Android NDK by osm0sis @ xda-developers 模块

## 使用建议

- 启动服务器时，请确保已有互联网连接。否则会导致无法获取上游服务器的IP。
- 如果出现异常，使用 `smartdns -clean` 命令可使防火墙恢复至开机状态。
...
位于 /data/adb/modules/smartdns/lib.sh
...
/data/adb/modules/smartdns/tools 为预留的启动脚本路径

tools 参数: disable 禁用 / 留空 执行路径目录中的sh / 指定文件

示例 [config_update](https://github.com/Aefer/smartdns-Android/blob/master/configupdate.sh) | Aefer

## SmartDNS 配置文件

位于 /data/media/0/Android/smartdns 文件夹，读取 ./smartdns.conf 文件
...
|log-file|日志文件路径|/dev/smartdns/config/log/smartdns.log|路径|log-file /var/log/smartdns.log
|log-size|日志大小|128K|数字+K,M,G|log-size 128K
|log-num|日志归档个数|2|数字|log-num 2
|audit-enable|设置审计启用|no|[yes\|no]|audit-enable yes
|audit-file|审计文件路径|/dev/smartdns/config/log/smartdns-audit.log|路径|audit-file /var/log/smartdns-audit.log
...
|ca-file|证书文件|/dev/smartdns/binary/CA/ca-certificates.crt|路径|ca-file /etc/ssl/certs/ca-certificates.crt
|ca-path|证书文件路径|/dev/smartdns/binary/CA|路径|ca-path /etc/ssl/certs
