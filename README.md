...
- Busybox for Android NDK by osm0sis @ xda-developers
...
- 启动服务器时，请确保有互联网连接。否则会导致无法获取上游服务器 域名IP
- 上游服务器优先指定 IP。一般同一个上游没必要配置不同协议
- 对国内用 udp, 移动用 tcp 可增加 IP 黑名单，对国外优先用 tls, https
...
|log-file|日志文件路径|/dev/smartdns/config/log/smartdns.log|路径|log-file /dev/smartdns/config/log/smartdns.log
|log-size|日志大小|128K|数字+K,M,G|log-size 128K
|log-num|日志归档个数|2|数字|log-num 2
|audit-enable|设置审计启用|no|[yes\|no]|audit-enable yes
|audit-file|审计文件路径|/dev/smartdns/config/log/smartdns-audit.log|路径|audit-file /dev/smartdns/config/log/smartdns-audit.log
